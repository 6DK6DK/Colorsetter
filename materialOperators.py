    ...
    def execute(self, context):
        ...
        with open(self.filepath, 'rb') as colorset:
            ...
            for i in range(16):
                ...
                context.object.active_material.cs_rows[i].gloss = self.read_i(colorset, 1)[0] * 255
                glow = self.read_i(colorset, 3)
                glow.append(1.0)
                context.object.active_material.cs_rows[i].glow = glow
                context.object.active_material.cs_rows[i].tile_id = self.read_i(colorset, 1)[0] * 255
                context.object.active_material.cs_rows[i].tile_transform = [x * 255 for x in self.read_i(colorset, 4)]
                if colorsetdat:
